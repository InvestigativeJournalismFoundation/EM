...
def train(dataset: str, lm: str = None, n_epochs: int = None, model_tag: str = None) -> str:
    ...
    effective_epochs = n_epochs if n_epochs is not None else int(tcfg.get("n_epochs", 10))
    ...
    out_dir = Path(to_abs(f"models/{dataset}_{tag}"))
...
def main() -> None:
    ...
    args = ap.parse_args()
    train(args.dataset, lm=args.lm, n_epochs=args.n_epochs, model_tag=args.model_tag)
